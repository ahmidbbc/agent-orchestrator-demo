...
- `POST /tasks` — creates a task from a JSON request body:

  ```json
  { "title": "Write demo script" }
  ```

  Responds with `201 Created` and the created task:

  ```json
  { "id": "generated-task-id", "title": "Write demo script" }
  ```
